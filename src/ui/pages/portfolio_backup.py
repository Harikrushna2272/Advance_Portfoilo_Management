...
def render_positions_table():
    ...
    # Update positions with current prices from yfinance
    if positions:
        try:
            ...
            for symbol in positions.keys():
                try:
                    ...
                    if current_price > 0:
                        ...
                        # Update market value
                        positions[symbol]["market_value"] = shares * current_price
                        ...
                        positions[symbol]["unrealized_pnl"] = unrealized_pnl
                        positions[symbol]["unrealized_pnl_pct"] = unrealized_pnl_pct
                except Exception as e:
                    print(f"Could not update price for {symbol}: {e}")
                    continue
        except Exception as e:
            print(f"Could not update prices: {e}")

    # If no positions, show message instead of sample data
    if not positions:
        st.info(
            "💡 No positions currently held. Use the manual trade execution below to open a position."
        )
        return
...
def render_trade_history():
    ...
    if not trades:
        st.info("💡 No trade history yet. Execute trades to see them appear here.")
        return
...
def render_allocation_charts():
    ...
    if not values or sum(values) == 0:
        st.info("💡 No portfolio data yet. Execute trades to see allocation.")
        return
    ...
    with st.expander("🔧 Execute Manual Trade", expanded=False):
        ...
        with col2:
            if st.button(
                ...
                success, message, order_id, executed_price = execute_manual_trade(
                    symbol=trade_symbol,
                    action=trade_action,
...
def execute_manual_trade(
    ...
    try:
        ...
        print(f"Error in execute_manual_trade: {traceback.format_exc()}")
