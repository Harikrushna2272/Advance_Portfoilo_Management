...
from src.utils.data_preprocessor import preprocess_for_rl
import os
...
class DecisionEngine:
    ...
    def load_ensemble_model(self):
        ...
        try:
            # Use new RLEnsemble class
            self.ensemble_model = RLEnsemble()

            if self.ensemble_model.is_ready():
                loaded_models = self.ensemble_model.get_loaded_models()
                print(f"✅ 5-Model Ensemble RL loaded successfully: {loaded_models}")
                return True
            else:
                print("⚠️  Warning: No RL models loaded, ensemble will not function")
                return False
            ...
            if self.ensemble_model is None:
                if not self.load_ensemble_model():
                    analysis_results["ensemble_rl"] = {
                        "error": "Failed to load ensemble model"
                    }
                    return analysis_results

            # Get preprocessed features for RL
            rl_features = preprocess_for_rl(stock_data)
    ...
    def _generate_final_decision(self, analysis_results: Dict) -> Dict:
        ...
        try:
            ...
            for agent_name in core_agents:
                if agent_name in analysis_results["agents"]:
                    ...
                    if isinstance(result, dict) and "signal" in result:
                        ...
                        print(
                            f"📊 {agent_name.title()}: {result['signal']} ({result.get('confidence', 50)}%)"
                        )
            ...
            final_decision = self._portfolio_manager_decision(
                combined_signals, analysis_results
            )
            ...
            return {"signal": "HOLD", "confidence": 0, "quantity": 0, "error": str(e)}
    ...
    def _get_rl_ensemble_decision(self, analysis_results: Dict) -> Dict:
        ...
        try:
            ...
            if (
                "ensemble_rl" not in analysis_results
                or "error" in analysis_results["ensemble_rl"]
            ):
            ...
            print(
                f"🎯 RL Ensemble: {rl_signal} (confidence: {rl_confidence}%, score: {weighted_score:.3f})"
            )
    ...
    def _portfolio_manager_decision(
        self, combined_signals: Dict, analysis_results: Dict
    ) -> Dict:
        """
        ...
        try:
            ...
            avg_agent_confidence = (
                total_confidence / len(agent_signals) if agent_signals else 50
            )

            # Determine agent consensus
            if (
                signal_counts["bullish"] > signal_counts["bearish"]
                and signal_counts["bullish"] > signal_counts["neutral"]
            ):
                agent_consensus = "bullish"
            elif (
                signal_counts["bearish"] > signal_counts["bullish"]
                and signal_counts["bearish"] > signal_counts["neutral"]
            ):
            ...
            if agent_consensus == "bullish" and rl_signal == "BUY":
                ...
                final_confidence = min(
                    90, (avg_agent_confidence + rl_confidence) / 2 * 0.8
                )
            ...
            quantity = int(
                base_quantity * (final_confidence / 100) * quantity_multiplier
            )
            ...
            return {
                ...
                "reasoning": self._generate_portfolio_reasoning(
                    agent_consensus, rl_signal, signal_counts, final_confidence
                ),
            }
        ...
        try:
            ...
            if agent_consensus == "bullish":
                reasoning_parts.append(
                    f"Strong bullish consensus from agents ({signal_counts['bullish']} bullish signals)"
                )
            elif agent_consensus == "bearish":
                reasoning_parts.append(
                    f"Strong bearish consensus from agents ({signal_counts['bearish']} bearish signals)"
                )
            else:
                reasoning_parts.append(
                    f"Mixed signals from agents (bullish: {signal_counts['bullish']}, bearish: {signal_counts['bearish']}, neutral: {signal_counts['neutral']})"
                )
            ...
            if agent_consensus == "bullish" and rl_signal == "BUY":
                reasoning_parts.append(
                    "Strong alignment between agents and RL - high confidence trade"
                )
            elif agent_consensus == "bearish" and rl_signal == "SELL":
                reasoning_parts.append(
                    "Strong alignment between agents and RL - high confidence trade"
                )
            elif agent_consensus in ["bullish", "bearish"] or rl_signal in [
                "BUY",
                "SELL",
            ]:
    ...
    def _calculate_position_size(
        self, confidence: float, analysis_results: Dict
    ) -> float:
        ...
        try:
            ...
            risk_signal = (
                analysis_results["agents"].get("risk", {}).get("signal", "neutral")
            )
            if risk_signal == "bearish":
        ...
        try:
            ...
            for agent_name, result in analysis_results["agents"].items():
                if isinstance(result, dict) and "signal" in result:
                    ...
                    reasoning_parts.append(
                        f"{agent_name.title()}: {signal} ({confidence}%)"
                    )

            # Add ensemble RL insight
            if (
                "ensemble_rl" in analysis_results
                and "signal" in analysis_results["ensemble_rl"]
            ):
            ...
            if final_signal == "BUY":
                reasoning_parts.append(
                    "Strong bullish consensus across multiple agents"
                )
            elif final_signal == "SELL":
                reasoning_parts.append(
                    "Strong bearish consensus across multiple agents"
                )
            else:
                reasoning_parts.append(
                    "Mixed signals or insufficient confidence for directional trade"
                )
        ...
        try:
            ...
            return {
                ...
                "average_confidence": np.mean(
                    [d["final_decision"].get("confidence", 0) for d in recent_decisions]
                ),
                "signal_distribution": {
                    "BUY": sum(
                        1
                        for d in recent_decisions
                        if d["final_decision"].get("signal") == "BUY"
                    ),
                    "SELL": sum(
                        1
                        for d in recent_decisions
                        if d["final_decision"].get("signal") == "SELL"
                    ),
                    "HOLD": sum(
                        1
                        for d in recent_decisions
                        if d["final_decision"].get("signal") == "HOLD"
                    ),
                },
            }
