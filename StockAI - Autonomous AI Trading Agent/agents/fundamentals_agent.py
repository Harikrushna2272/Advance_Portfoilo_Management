...
# Import the API functions from ai-hedge-fund (assume they are copied or available in the utils directory)
from utils import api as hedge_api

def analyze_fundamentals(stock, end_date):
    """Run fundamental analysis for a single stock."""
    try:
        metrics = hedge_api.get_financial_metrics(stock, end_date)
        if not metrics:
        ...
        print(f"Error in fundamental analysis for {stock}: {e}")
        return {"signal": "neutral", "confidence": 0}
